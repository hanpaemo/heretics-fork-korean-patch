...
### v2.2 (2026-03-13)
- 슬롯 면적순 배정으로 한글 텍스트 선명도 개선
- non-ASCII 영역 바운딩박스 클리어 + ASCII 패딩 복원으로 일본어 잔상 완전 제거
- 디버그 텍스처 출력 추가

### v2.0 (2026-03-13)
- data.win FONT/TXTR 직접 패치 방식으로 변경 — 한글 글리프가 정상 렌더링됨
- GameMaker fioq (커스텀 QOI) + BZ2 텍스처 디코더/인코더 자체 구현
- font_main_ja, font_menu_ja 양쪽 폰트 모두 패치
- 원본 CJK 글리프 메트릭(shift, offset)에 맞춘 한글 렌더링
